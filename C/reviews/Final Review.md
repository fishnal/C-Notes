# <a id="home-page-top"></a> CMSC 216H Midterm #2 Review
...
+ [Signals](#signals)
+ [Processes](#processes)
	+ [Multitasking](#processes-multitask)
	+ [User/Kernel Modes](#processes-modes)
	+ [Process Life Cycle](#processes-lifecyle)
	+ [Unix I/O](#processes-io)
	+ [Methods](#processes-methods)
+ [Threads](#threads)
+ [Time](#time)
+ [Static and Dynamic/Shared Libraries](#libraries)
...
+ <a id="unix-ln"></a> `ln` makes links between files
	+ Usage: `ln [opts] <target name> <link name>`
		+ `target name` is what you want to link
		+ `link name` is what you want to link to
	+ `-s` make symbolic links instead of hard ones
		+ `ln -s real_file.txt foo/link_file.txt`
...
+ <a id="clang-string-memset"></a> `memset(void *s, int c, size_t n)`
	+ Fills first `n` bytes of `s` with the constant byte `c`
...
|`LDD`|`ldd rd (YZ)+k`|`rd = *(YZ + k)`|Y,Z|k = [0,63]|
|`LDS`|`lds rd, addr`|`rd = *(addr)`|ALL||
|`ST`|`st -(XYZ)+, rr`|`--*(XYZ)++ = rr`|X,Y,Z|pre-dec, none, post-inc|
|`STD`|`std (YZ)+k, rr`|`*(YZ + k) = rr`|Y,Z|k = [0,63]|
|`STS`|`sts addr, rr`|`*(addr) = rr`|ALL|
|`TST`|`tst rd`|`rd == 0`|ALL||
|`NOP`|`nop`|||literally nothing|

## <a id="signals"></a> Signals

Signals interrupt a process. When set with a handler, you can *handle* whatever signal was just sent.

We can send signals to any process so long as we know it's process id via `kill()`, which takes in an process id and a signal to send. Note that the the function name `kill` does **NOT** mean, ironically, it kills the process we're sending the signal to:

```c
kill(pid_t pid, int signal);
```

We can also handle signals using the `signal()` function, which takes in an `int` indicating the signal and a function pointer taking an `int` returning `void`:

```c
signal(int signal, void (*handler)(int));
```

The following example shows that even when we send a signal to the current process, execution *can* still resume:

```c
void my_handler(int signal) {
  printf("Process not killed yet\n");
}
...
/* OUTPUT:
Process not killed yet
Exiting Process now
 */
```

Execution may not resume depending on how the signal that was caught is handled. Sometimes you may want to have your process exit if you catch a certain signal, it just depends on the context.

## <a id="memmap"></a> Memory Maps

Don't really know how to explain memory maps, so uh, just refer to <a href="http://www.cs.umd.edu/class/spring2018/cmsc216/exams/exam1/MemoryMapExample.pdf" target="_blank">this memory map example</a> and <a href="http://www.cs.umd.edu/class/spring2018/cmsc216/exams/final/ProcessDiagramExample.pdf">this process diagram example</a>
